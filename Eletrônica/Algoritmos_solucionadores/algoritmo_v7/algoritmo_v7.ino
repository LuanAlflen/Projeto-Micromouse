...
const float DISTANCE_FOR_CURVE = 17.0;
...
void loop(){
  while(!done){
      ...
      case RUN:
        ...
        if(next_step == ANTICOLISION and dsValues[0] < DISTANCE_FOR_CURVE){
          ...
          if(target != new_direction and countRun == 0 and next_step == CURVE){
            ...
            if(next_step == CURVE){
              run_delay = 1.13; //Estava em 1.13
            }else{
              run_delay = 1.3; //Estava em 1.0
            }
    ...
    analogWrite(MOTOR_IN_A1,0); 
    analogWrite(MOTOR_IN_A2,0);
    analogWrite(MOTOR_IN_B1,0);
    analogWrite(MOTOR_IN_B2,0);
