...
void loop(){
  while(!done){
    ...
    //armazena valores do sensor na fila circular
    ...
    switch (step) {
      case RUN:
        ...
        if(target != new_direction and countRun == 0 and next_step == CURVE){
          ...
          //Se não tiver uma parede, espera o delay e faz a curva
          if( dsValues[1] > 20 && turning_delay > run_delay ){
              identified_curve_time = t;
              step = next_step;
          //Mas se tiver uma parede, ignora o timer e faz a curva quando chega perto dela
          }else if (dsValues[1] < 5){
              identified_curve_time = t;
              step = next_step;      
          }
